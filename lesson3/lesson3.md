...
Articulation Root добавляется один раз на корневой объект всей конструкции — на Xform, не на звенья.
...
![](assets/robot_wheels.gif)

---

## Шаг 4 — Суставы и Запуск
...
![](assets/robot_joints.gif)

Если корпус улетает вверх — Body 0 не назначен в одном из суставов.
Если всё разваливается — проверь иерархию: Cube и оба Cylinder должны быть прямо внутри `Robot`, не друг в друге.
...
`DynamicCuboid` — куб с уже встроенными Rigid Body и Collision. `Dynamic` значит физический — то же что `Rigid Body with Colliders Preset` из урока 2.
...
`i * 0.8` — каждый следующий куб смещается на 0.8м по X. Так они встают в ряд а не друг в друге.
...
Собери двухколёсного робота по инструкции из части 2. Убедись что иерархия правильная — Cube и оба Cylinder прямо внутри Robot. Назначь Body 0 и Body 1 в обоих суставах. Проверь что Joint State Publisher двигает колёса слайдерами.

<details>
<summary>Подсказка</summary>

Самые частые ошибки:
- Body 0 не назначен — корпус улетает вверх
...
Пересматриваем часть 2 если что-то не получается
...
- При создании сустава: выбрать Cube, затем Cylinder через Ctrl
- В Property сустава обязательно назначить Body 0 (корпус) и Body 1 (колесо)
- Joint State Publisher двигает суставы слайдерами без кода: `Isaac Utils → Joint State Publisher`

...
> **Важно:** нужна видеокарта NVIDIA с поддержкой RTX. Карты AMD, Intel и старые серии NVIDIA GTX не поддерживаются.
...
![Содержимое папки после распаковки](assets/zap.gif)

### Шаг 3 — Запустить установку

Открыть папку и дважды кликнуть на `post_install.bat`. Откроется чёрное окно — нужно дождаться завершения.
...
После нажатия Play включится физика и конус упадёт вниз под действием гравитации. Нажми Stop чтобы вернуть сцену в исходное состояние.
...
- Требует видеокарту NVIDIA RTX, минимум 8 ГБ VRAM и 32 ГБ ОЗУ
- Установка: распаковать архив, запустить `post_install.bat`, затем `isaac-sim.bat`
